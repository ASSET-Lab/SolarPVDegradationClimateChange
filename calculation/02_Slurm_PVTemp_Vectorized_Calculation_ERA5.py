...
## total 27 years of ERA5 reanalysis dataset
## from 1950-1976
...
def handle_calcu_single_year(gcm_path, model, year, mount_method, save_path):
    ...
    ds = xr.open_dataset(f'/scratch/negishi/wu2411/Scratch/era5/surface_solar_radiation_downwards_{year}.nc').chunk({'time':chunk_size})
    # dtemp = xr.open_dataset(f'/scratch/negishi/wu2411/Scratch/era5/2m_temperature_{year}.nc')["t2m"].chunk({'time':chunk_size})
    dtemp = xr.open_dataset(f'/scratch/negishi/wu2411/Scratch/era5/2m_temperature_{year}.nc')["t2m"].chunk({'time':chunk_size})
    uas = xr.open_dataset(f'/scratch/negishi/wu2411/Scratch/era5/10m_u_component_of_wind_{year}.nc')["u10"].chunk({'time':chunk_size})
    vas = xr.open_dataset(f'/scratch/negishi/wu2411/Scratch/era5/10m_v_component_of_wind_{year}.nc')["v10"].chunk({'time':chunk_size})
    ...
    print('saving PVTemp_CloseRack_hourly_era5')
    with ProgressBar():        
        pvtemp.astype('float32').to_netcdf(f'/scratch/negishi/wu2411/Scratch/Vct_result/PVTemp_{mount_method}_era5_{year}.nc')
    print('saving CummuCFs_CloseRack_hourly_era5')
    with ProgressBar():
        CapFact.astype('float32').to_netcdf(f'/scratch/negishi/wu2411/Scratch/Vct_result/CummuCFs_{mount_method}_era5_{year}.nc')
...
def main(mounting_config, years_list):

    scenarios = ['era5'] # 替换为您的具体情景
    years = years_list
    for year in years:
        print(f'handle_calcu_single_year_{year}')
        handle_calcu_single_year(scenarios[0],year,mounting_config)
...
if __name__ == "__main__":
    ...
    main(mounting_config, years_str_list)
